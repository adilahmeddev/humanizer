...
### 5. Headline, then the evidence

**Watch for:** a sentence shaped "[claim or framing]: [the evidence]"; the same shape split across two sentences ("Card failures drive cancellations. Among cancelled subscriptions, ..."); a fragment that only announces what follows ("Short answer:", "Two things stand out.", "Three things to take from it:"); a bullet whose first sentence is bold and the rest plain; a heading shaped "Topic: verdict"
**Problem:** The conclusion is stated as a headline and the facts hang off it, so every sentence reads like a slide title. Let the evidence lead and draw the point from it, or join the two with an ordinary connective (since, with, so). Remove a fragment that only announces. Unbold a headline sentence. A short label a document uses as its list format ("Next steps:") is format, not this tell.
**Before:**
> Referral rates are flat: 20 to 23% of customers in every region have referred someone.
**After:**
> Between 20 and 23% of customers in every region have referred someone, so referral rates are flat.
**Before (framing fragment):**
> Two things the other surveys add. Villages have the most repeat buyers, and towns sit in the low-margin middle.
**After:**
> Villages have the most repeat buyers, and towns sit in the low-margin middle.
...
**Watch for:** cut (for a segmentation), a read (for a finding), controls for, survives a control, holds X fixed, cohort, the population, the base, lens, carries signal, netting, over-index, bears on, and the equivalent shorthand of any other trade
**Problem:** Words a profession uses among itself land as jargon on the reader, and a model reaches for them because they are the likeliest words in that register. Say what the word means: "once postcode is taken into account", "customers who signed up two to four years ago", "the figures cover". Keep a term the reader's own field uses or the document defines.
**Before:**
> The last column holds the sign-up date fixed and the gap survives, so the uplift is not a cohort effect.
**After:**
> The last column only counts customers who signed up two to four years ago, and the gap is still there, so it is not explained by how long people have been customers.
